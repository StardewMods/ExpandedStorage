...
internal sealed class SearchMenu : BaseMenu
{
    ...
    private readonly ClickableTextureComponent scrollBarLeft;
    private readonly ClickableTextureComponent scrollBarRight;
    private readonly Rectangle scrollBarRunnerLeft;
    private readonly Rectangle scrollBarRunnerRight;
    private readonly SearchComponent searchBar;
    ...
    public SearchMenu(SearchHandler searchHandler)
    {
        ...
        this.SearchText = string.Empty;
        this.searchBar = new SearchComponent(
        ...
        this.scrollBarLeft = new ClickableTextureComponent(
            new Rectangle(
        ...
        this.scrollBarRight = new ClickableTextureComponent(
            new Rectangle(
                this.arrowUpRight.bounds.X + 12,
                this.arrowUpRight.bounds.Y + this.arrowUpRight.bounds.Height + 4,
                24,
                40),
            Game1.mouseCursors,
            new Rectangle(435, 463, 6, 10),
        ...
        this.scrollBarRunnerLeft = new Rectangle(
            this.scrollBarLeft.bounds.X,
            this.arrowUpLeft.bounds.Y + this.arrowUpLeft.bounds.Height + 4,
            this.scrollBarLeft.bounds.Width,
            scrollBarRunnerHeight);

        this.scrollBarRunnerRight = new Rectangle(
            this.scrollBarRight.bounds.X,
            this.arrowUpRight.bounds.Y + this.arrowUpRight.bounds.Height + 4,
            this.scrollBarRight.bounds.Width,
            scrollBarRunnerHeight);
    ...
    private string SearchText
    {
        ...
        set
        {
            ...
            this.rowsRight = (int)Math.Ceiling(
    ...
    public override void draw(SpriteBatch b, int red = -1, int green = -1, int blue = -1)
    {
        ...
        IClickableMenu.drawTextureBox(
            ...
            this.scrollBarRunnerLeft.X,
            this.scrollBarRunnerLeft.Y,
            this.scrollBarRunnerLeft.Width,
            this.scrollBarRunnerLeft.Height,
            Color.White,
            4f);
        ...
        IClickableMenu.drawTextureBox(
            ...
            Game1.mouseCursors,
            new Rectangle(403, 383, 6, 6),
            this.scrollBarRunnerRight.X,
            this.scrollBarRunnerRight.Y,
            this.scrollBarRunnerRight.Width,
            this.scrollBarRunnerRight.Height,
            Color.White,
            4f);

        this.scrollBarLeft.draw(b);
        this.scrollBarRight.draw(b);

        this.arrowUpLeft.draw(b, this.scrollLeft > 0 ? Color.White : Color.Gray * 0.5f, 1f);
        this.arrowUpRight.draw(b, this.scrollRight > 0 ? Color.White : Color.Gray * 0.5f, 1f);
        this.arrowDownLeft.draw(b, this.scrollLeft < this.rowsLeft - 1 ? Color.White : Color.Gray * 0.5f, 1f);
        this.arrowDownRight.draw(
            b,
            this.scrollRight < this.rowsRight - this.inventory.rows - 1 ? Color.White : Color.Gray * 0.5f,
            1f);
    ...
    public override void performHoverAction(int x, int y)
    {
        ...
        if (this.scrollLeft < this.rowsLeft - 1)
        {
        ...
        if (this.scrollRight < this.rowsRight - 1)
        {
    ...
    public override void receiveLeftClick(int x, int y, bool playSound = true)
    {
        ...
        if (this.scrollLeft > 0 && this.arrowUpLeft.containsPoint(x, y))
        {
        ...
        if (this.scrollRight > 0 && this.arrowUpRight.containsPoint(x, y))
        {
            Game1.playSound("shwip");
            this.scrollRight--;
            this.inventory.actualInventory =
                this.allItems.Skip(this.scrollRight).Take(this.inventory.capacity).ToList();

            return;
        }

        if (this.scrollLeft < this.rowsLeft - 1 && this.arrowDownLeft.containsPoint(x, y))
        {
            Game1.playSound("shwip");
            this.scrollLeft++;
            return;
        ...
        if (this.scrollRight < this.rowsRight - this.inventory.rows - 1 && this.arrowDownRight.containsPoint(x, y))
        {
            Game1.playSound("shwip");
            this.scrollRight++;
            this.inventory.actualInventory = this
                .allItems.Skip(this.scrollRight * (this.inventory.capacity / this.inventory.rows))
        ...
        if (this.scrollBarLeft.containsPoint(x, y))
        {
            return;
        }

        if (this.scrollBarRight.containsPoint(x, y)) { }
    }
